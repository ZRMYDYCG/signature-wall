...
watch(visible, (newVisible) => {
  if (newVisible) {
    ...
    document.body.style.paddingRight = '10px' // 防止内容偏移
  }
  else {
    // 恢复页面滚动
...
<template>
  ...
  <transition name="fade">
    <div v-if="visible" class="fixed z-[9999] left-[50%] top-[50%] translate-x-[-50%] translate-y-[-50%]">
      <div
        class="relative flex h-[20em] w-[25em] flex-col items-center justify-start overflow-clip rounded-[1.5em] bg-[#B7DFFB] px-[1em] py-[1.5em] shadow-[0px_2px_2px_0px_#1a4766,0px_2px_8px_0px_#3083bb]"
        @click.stop
      >
        <!-- 头部区域 -->
